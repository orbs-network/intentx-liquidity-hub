...
const AdvancedBarCumulativeChart = ({ data, chartTitle, leftAxisLabel, isUSD = false, seriesName }: any) => {
    ...
    if (isMonthView && data) {
      ...
      data.forEach((entry) => {
        ...
        if (!monthlyTotals[yearMonthKey]) {
          monthlyTotals[yearMonthKey] = { value: 0, cumulative: 0, time: date }
        }
        monthlyTotals[yearMonthKey].value += entry.value
       

        if (!monthlyTotals[yearMonthKey] || entry.cumulative > monthlyTotals[yearMonthKey].cumulative) {
          monthlyTotals[yearMonthKey].cumulative = entry.cumulative
          // monthlyTotals[yearMonthKey] = { value: entry.value, cumulative: entry.cumulative, time: date }
        }
      })
  ...
  const yAxisFormatter = (value: number) => {
    ...
    if (value >= 1000000000) {
      const formattedValue = (value / 1000000000).toFixed(2) + 'B'
  ...
  const options = {
    ...
    yAxis: [
      {
        ...
        axisLabel: {
          formatter: (value: any) => {
            if (value >= 1000000000) {
              const formattedValue = (value / 1000000000).toFixed(0) + 'B'
              return isUSD ? '$' + formattedValue : formattedValue
            } else if (value >= 1000000) {
    ...
    series: [
      ...
      {
        data: seriesData?.map((element) => [new Date(element.time), element.cumulative]),
        type: 'line',
        smooth: true,
        name: 'cumulative',
        yAxisIndex: 1,
...
export default AdvancedBarCumulativeChart
