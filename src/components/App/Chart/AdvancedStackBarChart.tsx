...
const AdvancedStackBarChart = ({ data, chartTitle, leftAxisLabel }: any) => {
    ...
    if (isMonthView && data) {
      ...
      data.forEach((entry) => {
        ...
        if (!monthlyTotals[yearMonthKey]) {
          monthlyTotals[yearMonthKey] = { deposit: 0, withdraw: 0, time: date }
        }
        monthlyTotals[yearMonthKey].deposit += entry.deposit
        monthlyTotals[yearMonthKey].withdraw += entry.withdraw
      })
      ...
      setSeriesData(
        data?.map((element) => {
          const timestamp = parseInt(element.time, 10)
          const date = new Date(timestamp * 1000)
  ...
  const yAxisFormatter = (value: number) => {
    ...
    if (absValue >= 1000000000) {
      return (value < 0 ? '- $' : '$') + (absValue / 1000000000).toFixed(2) + 'B'
  ...
  const options = {
    ...
    series: [
      {
        name: 'deposit',
        data: seriesData?.map((element) => [element.time, element.deposit]),
        type: 'bar',
        stack: 'total',
      },
      {
        name: 'withdraw',
        data: seriesData?.map((element) => [element.time, element.withdraw]),
        type: 'bar',
        stack: 'total',
      },
    ],
...
export default AdvancedStackBarChart
