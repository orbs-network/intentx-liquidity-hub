...
const InsufficientAmountLabel = styled.div``
...
export default function ErrorButton({
  ...
  const text = useMemo(() => {
    ...
    if (state === ErrorState.INSUFFICIENT_BALANCE) return 'Insufficient Balance'
    else if (state === ErrorState.AMOUNT_IS_ZERO) return "Amount can't be zero"
    else if (state === ErrorState.LESS_THAN_MIN_ACCEPTABLE_QUOTE_VALUE) return 'Amount is too low'
    else if (state === ErrorState.OUT_OF_RANGE_PRICE) return 'Price is out of range'
    else if (state === ErrorState.REMAINING_AMOUNT_UNDER_10) return 'Amount is too high'
    else if (state === ErrorState.PARTIAL_CLOSE_WITH_SLIPPAGE) return 'Liquidation after close!'
    ...
    else if (state === ErrorState.CAP_REACHED) return 'No Liquidity Available'
    else if (state === ErrorState.HIGHER_THAN_MAX_NOTIONAL_VALUE) return 'Higher than max notional value'
