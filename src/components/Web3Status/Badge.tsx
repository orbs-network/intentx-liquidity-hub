...
import { useMax20ClosedQuotes, useQuotesPnl } from 'hooks/useBadgesData'
...
export default function Badge() {
  ...
  const navStatusIconSrc = useMemo(() => {
    ...
    if (healthNumber && healthNumber !== Infinity && !isNaN(healthNumber) && healthNumber > 200) {
      setTooltipTitle('Medic')
      setTooltipText('For users with a robust account health above 200%. Your portfolio is as strong as your strategy.')
      return Medic
    }
    if (isArcher) {
      setTooltipTitle('Archer')
      setTooltipText('Earn this badge by making profit in at least 5 different markets in your last 20 trades.')
      return Archer
    }
    if (isSniper) {
      setTooltipTitle('Sniper')
      setTooltipText('Be a sharpshooter by closing 80% of your last 20 trades with profit.')
      return Sniper
    }
    return ''
    ...
    <span>
      {navStatusIconSrc && (
        <a data-tip data-for={'badge'}>
          <Image unoptimized={true} src={navStatusIconSrc} alt={'badge'} />
