...
import { ToolTipBottomEnd, ToolTipLeft } from 'components/ToolTip'
import { MAX_SLIPPAGE, MIN_SLIPPAGE } from 'constants/slippage'
import { useCallback, useState } from 'react'
import { useDispatch } from 'react-redux'
import { setHideFavouriteBar, setTradeContainerIsReverse } from 'state/user/actions'
import {
    useActiveTradeView,
    useFillOrKillMode,
    useHideFavouriteBar,
    useSetActiveTradeView,
    useSetFillOrKillMode,
    useSetSlippageToleranceCallback,
    useSlippageTolerance,
    useTradeContainerIsReverse,
...
export default function SettingModal() {
  const isOpen = useModalOpen(ApplicationModal.SETTING)
  const toggleModal = useToggleModal(ApplicationModal.SETTING)
  ...
  const tradeContainerIsReverse = useTradeContainerIsReverse()
  ...
  const setFillOrKillMode = useSetFillOrKillMode()
    ...
    <Modal isOpen={isOpen} onBackgroundClick={toggleModal} onEscapeKeydown={toggleModal}>
      <Wrapper>
        ...
        <Gas>
          ...
          <StyledInfoIcon data-tip data-for="trading-console-position-settings-tooltip" />
          <ToolTipLeft id="trading-console-position-settings-tooltip" aria-haspopup="true">
            Switch between left and right layout <br /> positions for the trading console
          </ToolTipLeft>
        </Gas>
        <ButtonGroup>
          <WrapperButtonTraddingConsole>
            <ButtonTraddingConsole
              direction="left"
              active={!tradeContainerIsReverse}
              onClick={() => dispatch(setTradeContainerIsReverse(false))}
            />
            <span>Left</span>
          </WrapperButtonTraddingConsole>
          <WrapperButtonTraddingConsole>
            <ButtonTraddingConsole
              direction="right"
              active={tradeContainerIsReverse}
              onClick={() => dispatch(setTradeContainerIsReverse(true))}
            />
            <span>Right</span>
          </WrapperButtonTraddingConsole>
          {/* <GradientButton
            buttonFilled={!tradeContainerIsReverse}
            height="42px"
            label={'Left'}
            onClick={() => dispatch(setTradeContainerIsReverse(false))}
          />
        ...
        <ToggleGroup>
          <TogleItem>
            <span>Show Favorites Bar</span>

            {/* <Checkbox
              checked={!isFavouriteBarHidden}
              onChange={() => {
                dispatch(setHideFavouriteBar(!isFavouriteBarHidden))
              }}
              label=""
              id="show-favourites-bar-checkbox"
              name="show-favourites-bar"
            /> */}
            <Switch
              value={!isFavouriteBarHidden}
              onChange={() => {
                dispatch(setHideFavouriteBar(!isFavouriteBarHidden))
              }}
            />
          ...
          <TogleItem>
            <span>Enable Depth Chart</span>

            {/* <Checkbox
            checked={activeTradeView === TradeView.TRADE}
            onChange={() => {
              setActiveTradeView(activeTradeView === TradeView.TRADE_FULL ? TradeView.TRADE : TradeView.TRADE_FULL)
            }}
            label=""
            id="enable-depth-chart-checkbox"
            name="enable-depth-chart"
          /> */}
            <Switch
              value={activeTradeView === TradeView.TRADE}
              onChange={() => {
                setActiveTradeView(activeTradeView === TradeView.TRADE_FULL ? TradeView.TRADE : TradeView.TRADE_FULL)
              }}
            />
          ...
          <TogleItem>
            ...
            >
              <span>Enable Fill Or Kill</span>
              <div
                style={{
                  marginLeft: '5px',
                }}
              >
                <StyledInfoIcon data-tip data-for="fill-or-kill-tooltip" />
              </div>
              <ToolTipBottomEnd id="fill-or-kill-tooltip" aria-haspopup="true">
                If enabled, your transaction will revert if the full amount cannot be traded.
              </ToolTipBottomEnd>
            </div>

            <Switch
              value={fillOrKillMode}
              onChange={() => {
                setFillOrKillMode(!fillOrKillMode)
              }}
            />
          </TogleItem>
        </ToggleGroup>
