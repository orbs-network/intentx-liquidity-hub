...
export default function Web3Network() {
  ...
  return isMobile ? (
    <>
      <NavigationButton onClick={onClickButton}>
        <ImageWithFallback src={Chain.logoUrl} alt={Chain.label} width={20} height={20} />
    ...
    <Container ref={ref}>
      ...
      <NavigationButton onClick={onClickButton} width={isMultiChain ? '52px' : undefined}>
        <ImageWithFallback src={Chain.logoUrl} alt={Chain.label} width={20} height={20} />
        {isMultiChain && <Chevron open={networksModal} />}
      </NavigationButton>
    </Container>
