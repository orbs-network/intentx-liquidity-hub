...
import type { LOGIN_TYPE } from '@toruslabs/customauth'
...
export abstract class AuthKitBasePack {
   ...
   */
  abstract signIn(verifier: string): Promise<AuthKitSignInData>

  /**
   * Refresh the session in the pack
   * @returns The sign in data from the provider
   */
  abstract refreshSession(): Promise<AuthKitSignInData>
  ...
  async getAddress(): Promise<string> {
    ...
    const ethersProvider = new ethers.providers.Web3Provider(this.getProvider() as ethers.providers.ExternalProvider)
  ...
  #getApiKit(txServiceUrl: string): SafeApiKit {
    ...
    const provider = new ethers.providers.Web3Provider(this.getProvider() as ethers.providers.ExternalProvider)
