...
export class GelatoRelayPack implements RelayPack {
  ...
  async getEstimateFee(chainId: number, gasLimit: string, gasToken?: string): Promise<string> {
    ...
    const estimation = await this.#gelatoRelay.getEstimatedFee(BigInt(chainId), feeToken, BigInt(gasLimit), false)
   ...
   *
   */
  private async createPaymentToGelato(safe: Safe, gas: string, options: MetaTransactionOptions): Promise<Transaction> {
    ...
    const transferToGelato = createERC20TokenTransferTransaction(gasToken, gelatoAddress, paymentToGelato)
  ...
  private async createTransactionWithTransfer({
    ...
    if (gasLimit) {
      ...
      const syncTransaction = await safe.createTransaction({
        safeTransactionData: [transferToGelato, ...transactions],
        onlyCalls,
        options: {
          nonce,
          gasToken,
  ...
  async sendSponsorTransaction(target: string, encodedTransaction: string, chainId: number): Promise<RelayResponse> {
    if (!this.#apiKey) {
  ...
  async executeRelayTransaction(
    ...
    console.log('Encoded transaction ', safeTransactionEncodedData)
