...
const mapRequestToStruct = (request: SponsoredCallRequest): SafeRequestPayload<BaseRelayParams> => {
  return {
    chainId: request.chainId.toString(),
    target: utils.getAddress(request.target as string),
    data: request.data,
  }
}
...
const sponsoredCall = async (
  ...
  try {
    ...
    const postResponse = await post<BaseRelayParams & RelayRequestOptions & ApiKey, RelayResponse>(
      {
