...
export async function mapRequestToStruct(
  ...
  override: Partial<CallWithERC2771RequestOptionalParameters | CallWithConcurrentERC2771RequestOptionalParameters>
  ...
  if (isConcurrentRequest(request)) {
    const concurrentOverride = override as Partial<CallWithConcurrentERC2771RequestOptionalParameters>
    return {
    ...
    const nonConcurrentOverride = override as Partial<CallWithERC2771RequestOptionalParameters>
    if (nonConcurrentOverride.userNonce === undefined && request.userNonce === undefined) {
    ...
    return {
      ...
      data: request.data,
      user: getAddress(request.user as string),
      userNonce: nonConcurrentOverride.userNonce !== undefined ? nonConcurrentOverride.userNonce : request.userNonce,
