...
import { Config, EIP712_DOMAIN_TYPE_DATA, SafeRequestPayload } from '../../types'
import {
...
export function getPayloadToSign(
  ...
  config: Config
): SponsoredCallConcurrentERC2771PayloadToSign | CallWithSyncFeeConcurrentERC2771PayloadToSign
...
export function getPayloadToSign(
  ...
  const domain = getEIP712Domain({ chainId: BigInt(struct.chainId), type }, config)
