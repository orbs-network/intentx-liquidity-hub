...
import { ApiKey, Config, Optional, RelayCall, RelayRequestOptions, RelayResponse } from '../types'
...
export const relayWithSyncFee = async (
  ...
  try {
    ...
    const isSupported = await isNetworkSupported({ chainId: request.chainId }, config)
    if (!isSupported) {
      throw new Error(`Chain id [${request.chainId}] is not supported`)
    }
    return await post<CallWithSyncFeeRequest & RelayRequestOptions & Optional<ApiKey, 'sponsorApiKey'>, RelayResponse>(
      {
    ...
    throw new Error(`GelatoRelaySDK/relayWithSyncFee: Failed with error: ${getHttpErrorMessage(error)}`)
  }
}
