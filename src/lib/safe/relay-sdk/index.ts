...
import { WebsocketHandler } from './utils/index'
...
export class GelatoRelay {
  ...
  private _getConfiguration = (config?: Partial<Config>): Config => {
    ...
    return {
      ...
      contract: {
        relayERC2771: config?.contract?.relayERC2771 ?? GELATO_RELAY_ERC2771_ADDRESS,
        relay1BalanceERC2771: config?.contract?.relay1BalanceERC2771 ?? GELATO_RELAY_1BALANCE_ERC2771_ADDRESS,
        relayERC2771zkSync: config?.contract?.relayERC2771zkSync ?? GELATO_RELAY_ERC2771_ZKSYNC_ADDRESS,
        relay1BalanceERC2771zkSync:
          config?.contract?.relay1BalanceERC2771zkSync ?? GELATO_RELAY_1BALANCE_ERC2771_ZKSYNC_ADDRESS,
        relayConcurrentERC2771: config?.contract?.relayConcurrentERC2771 ?? GELATO_RELAY_CONCURRENT_ERC2771_ADDRESS,
        relay1BalanceConcurrentERC2771:
          config?.contract?.relay1BalanceConcurrentERC2771 ?? GELATO_RELAY_1BALANCE_CONCURRENT_ERC2771_ADDRESS,
        relayConcurrentERC2771zkSync:
          config?.contract?.relayConcurrentERC2771zkSync ?? GELATO_RELAY_CONCURRENT_ERC2771_ZKSYNC_ADDRESS,
  ...
  callWithSyncFee = async (
    ...
    const response = await library.relayWithSyncFee({ request, sponsorApiKey, options }, this.#config)
  ...
  sponsoredCall = async (
    ...
    const response = await library.relayWithSponsoredCall({ request, sponsorApiKey, options }, this.#config)
  ...
  getSignatureDataERC2771 = (
    ...
    signerOrProvider: SignerOrProvider,
    type: ERC2771Type
  ): Promise<SignatureData> => library.getSignatureDataERC2771({ request, signerOrProvider, type }, this.#config)
  ...
  getDataToSignERC2771 = (
    ...
    type: ERC2771Type,
    signerOrProvider?: SignerOrProvider
  ): Promise<PayloadToSign> => library.getDataToSignERC2771({ request, signerOrProvider, type }, this.#config)
   ...
   */
  isNetworkSupported = (chainId: bigint): Promise<boolean> => library.isNetworkSupported({ chainId }, this.#config)

  /**
   * @returns {Promise<string[]>} List of networks where Relay V2 is supported
   */
  getSupportedNetworks = (): Promise<string[]> => library.getSupportedNetworks(this.#config)

  /**
   * @param {bigint} chainId - Chain Id
   * @returns {Promise<boolean>} Boolean to demonstrate if the oracle is active on the provided chain
   */
  isOracleActive = (chainId: bigint): Promise<boolean> => library.isOracleActive({ chainId }, this.#config)

  /**
   * @returns {Promise<string[]>} List of chain ids where the Gelato Oracle is active
   */
  getGelatoOracles = (): Promise<string[]> => library.getGelatoOracles(this.#config)
   ...
   *
   */
  getPaymentTokens = (chainId: bigint): Promise<string[]> => library.getPaymentTokens({ chainId }, this.#config)
  ...
  getEstimatedFee = (
    ...
    library.getEstimatedFee({ chainId, paymentToken, gasLimit, isHighPriority, gasLimitL1 }, this.#config)
