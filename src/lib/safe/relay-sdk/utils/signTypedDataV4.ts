...
export const signTypedDataV4 = async (
  ...
  if (isSigner(signerOrProvider)) {
    return await signerOrProvider.signTypedData(payload.domain, payload.types, payload.message)
  }
  ...
  const signature = await signerOrProvider.send(SIGN_TYPED_DATA_V4, [address, JSON.stringify(payload)])
