...
import { TransactionStatusResponse } from '../lib/status/types/index'

import { isFinalTaskState } from './isFinalTaskState'
import { ErrorWebsocketMessage, UpdateWebsocketMessage, WebsocketEvent, WebsocketMessage } from './websocketMessage'
...
export class WebsocketHandler {
  ...
  private async _ensureIsConnected(): Promise<boolean> {
    if (!this.#websocket) {
      this._connect()
    } else if (this.#websocket.readyState !== WebSocket.CONNECTING && this.#websocket.readyState !== WebSocket.OPEN) {
      this._reconnect()
    }
