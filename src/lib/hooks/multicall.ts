...
import { UseContractReadConfig, useContractRead, useContractReads } from 'wagmi'
...
export function useSingleContractMultipleData(
  ...
  const configs = callsData.map((args) => ({ ...contract, functionName, args }))
...
export function useSingleContractMultipleMethods(
  ...
  const configs = callsData?.map(({ callInputs: args, functionName }) => ({ ...contract, functionName, args }))
  return useContractReads({ contracts: configs, watch: true, ...{ option } })
}
...
export function useSingleCallResult(
  ...
  return useContractRead({ ...{ contract }, functionName, args: [...[callInputs]], watch: true, ...{ option } })
}
