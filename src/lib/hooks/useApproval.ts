...
import { calculateGasMargin2 } from 'utils/web3'
...
export function useApprovalStateForSpender(
  ...
  const { tokenAllowance, refetch } = useERC20Allowance({ token, owner, spender })
  ...
  }, [currency, refetch, amountToApprove, spender, tokenAllowance, pendingApproval])
}
...
export function useApproval(
  ...
  spender: string | undefined,
  useIsPendingApproval: (token?: Token, spender?: string) => boolean
): [ApprovalState, () => Promise<{ response: string; tokenAddress: string; spenderAddress: string } | undefined>] {
  const { chainId } = useActiveConnectionDetails()
  const token = currency?.isToken ? currency.wrapped : undefined

  // check the current approval status
  const [approvalState] = useApprovalStateForSpender(currency, amountToApprove, owner, spender, useIsPendingApproval)
  ...
  const approve = useCallback(async () => {
    ...
    let useExact = false
    const estimatedGas = await tokenContract.estimateGas.approve([spender as Address, MAX_UINT256]).catch(() => {
      // general fallback for tokens which restrict approval amounts
      useExact = true
      return tokenContract.estimateGas.approve([spender as Address, BigInt(amountToApproveBN)])
    })

    return tokenContract.write
      .approve([spender as Address, useExact ? BigInt(amountToApproveBN) : MAX_UINT256], {
        gas: calculateGasMargin2(estimatedGas),
      })
      .then((response) => {
        return {
          response,
