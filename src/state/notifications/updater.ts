...
import { ActionStatus, NotificationDetails, NotificationResponse, NotificationType, StateType } from './types'

import useNotificationHistory from 'lib/hooks/useNotificationHistory'

import { useAddPopup } from 'state/application/hooks'
import { useHedgerInfo } from 'state/hedger/hooks'
import { useActiveAccountAddress, useAddRejectedQuoteCallback, useUserWhitelist } from 'state/user/hooks'
...
function useNotificationsWebSocket() {
  ...
  const addRejectedQuoteCallback = useAddRejectedQuoteCallback()
    ...
    try {
      ...
      if (notification.notificationType !== NotificationType.OTHER) {
        ...
        if (notification.showInModal) {
          ...
          if (notification.notificationType === NotificationType.HEDGER_ERROR) {
            const quoteId = notification.quoteId
            // Updating rejected quotes state
            if (quoteId) {
              addRejectedQuoteCallback(quoteId)
            }
          }
        }
      }
  ...
  }, [addPopup, addRejectedQuoteCallback, lastJsonMessage, newNotificationNotifier, notificationAdder])
}
