  ...
  (builder) =>
    builder
      .addCase(addUnreadNotification, (state, { payload: { notification } }) => {
        ...
        if (existedNotification && notificationType !== NotificationType.TRANSFER_COLLATERAL) {
          remove(unreadNotification, { quoteId, notificationType, actionStatus })
        }
        if (readExistedNotification && notificationType !== NotificationType.TRANSFER_COLLATERAL) {
          remove(readNotification, { quoteId, notificationType, actionStatus })
        }
      ...
      .addCase(readOneNotification, (state, { payload: { notification } }) => {
        ...
        const readExistedNotification = find(readNotification, { quoteId, notificationType })
        if (!existedNotification && notificationType !== NotificationType.TRANSFER_COLLATERAL) {
      ...
      .addCase(getNotifications.fulfilled, (state, { payload: { unreadNotifications } }) => {
